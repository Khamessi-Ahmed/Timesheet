import { Component, ViewChild, type OnInit, type AfterViewInit } from "@angular/core"
...
@Component({
  ...
  templateUrl: "./man-timesheet.component.html",
  styleUrl: "./man-timesheet.component.css",
})
export class ManTimesheetComponent implements OnInit, AfterViewInit {
  ...
  constructor(
    // private http: HttpClient, // Removed as it is unused
  ...
  loadTimesheet(): void {
    ...
    this.timesheetService.getUserTimesheet(this.managerId).subscribe({
      next: (timesheet) => {
  ...
  loadManagerTasks(): void {
    this.loading = true
    this.tacheService.getTachesBymanager(this.managerId).subscribe({
      next: (tasks) => {
        if (Array.isArray(tasks)) {
          this.tasks = tasks
        } else {
      ...
      error: (err) => {
        console.error("Erreur lors du chargement des tâches du manager :", err)
        this.tasks = []
        this.loading = false
      },
    })
  ...
  loadConges(): void {
    if (!this.managerId) {
      console.error("Impossible de charger les congés : ID du manager manquant")
      return
    }

    this.loading = true
    this.timesheetService.getCongesByEmployee(this.managerId).subscribe({
      next: (conges) => {
  ...
  updateTaskOnCalendar(taskId: number, startDate: Date, endDate: Date): void {
    ...
    this.timesheetService
      .updateTaskStatus(taskId, this.managerId, {
  ...
  submitCongesForm(): void {
    ...
    this.timesheetService.addConges(this.managerId, congesData).subscribe({
      next: (response) => {
